...
export const Accounts: React.FC = () => {
    ...
    const handleGenerateStatement = () => {
        ...
        if (statementWindow) {
            const rows = selectedAccount.transactions.map(tx => `
            <tr>
                <td>${tx.date}</td>
                <td>${tx.description}</td>
                <td>${tx.reference || '-'}</td>
                <td style="text-align: right; color: ${tx.type === 'Credit' ? 'green' : 'red'}">
          ...
          `).join('');
            ...
            <html>
            <head>
                <title>Statement - ${selectedAccount.companyName}</title>
                <style>
                    ...
                    .account-details { margin-bottom: 30px; background: #f9f9f9; padding: 20px; border-radius: 8px; }
                    table { width: 100%; border-collapse: collapse; margin-top: 20px; }
                    th, td { border-bottom: 1px solid #ddd; padding: 12px; text-align: left; }
                    th { background-color: #f1f1f1; text-transform: uppercase; font-size: 12px; color: #555; }
                    .balance-box { text-align: right; margin-top: 30px; font-size: 18px; }
            ...
            <body>
                <div class="header">
                    <div class="company-info">
                        <h1>Shravya Tours</h1>
                        <p>123 Adventure Avenue, Mumbai, India</p>
                    </div>
                    <div class="statement-title">
                        LEDGER STATEMENT<br>
                        <span style="font-size: 14px; font-weight: normal;">Date: ${new Date().toLocaleDateString()}</span>
                    </div>
                </div>

                <div class="account-details">
                    <strong>Account:</strong> ${selectedAccount.companyName}<br>
                    <strong>Contact:</strong> ${selectedAccount.name} (${selectedAccount.phone})<br>
                    <strong>Type:</strong> ${selectedAccount.type} Partner
                </div>
                ...
                <div class="balance-box">
                    <strong>Net Balance:</strong> ₹${selectedAccount.currentBalance.toLocaleString()}
                </div>

                <div class="footer">
                    <p>Generated by Shravya Tours Admin System.</p>
                </div>
