...
export const Leads: React.FC = () => {
    ...
    const handleAnalyzeLead = async (lead: Lead) => {
        setAnalyzing(true);
        const toastId = toast.loading('Analyzing lead potential...');
        try {
            const result = await analyzeLead(lead);
            updateLead(lead.id, { aiScore: result.score, aiSummary: result.summary });
    ...
    const filteredLeads = leads.filter(l => {
        ...
        const matchesAssignment = !isRestricted || l.assignedTo === currentUser?.id;
    ...
    const handleBulkStatus = (status: string) => {
        selectedLeads.forEach(id => updateLead(id, { status: status as any }));
        setSelectedLeads([]);
        toast.success(`Leads marked as ${status}`);
    };

    const handleBulkAssign = (staffId: number) => {
        selectedLeads.forEach(id => updateLead(id, { assignedTo: staffId }));
        setSelectedLeads([]);
    ...
    return (
        <div className="flex h-full flex-col bg-slate-50 dark:bg-slate-900 relative">
            ...
            {selectedLeads.length > 0 && (
                <div className="absolute top-[80px] left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-4 py-2 rounded-xl shadow-2xl flex items-center gap-4 animate-in fade-in slide-in-from-top-4">
                    <span className="text-sm font-bold">{selectedLeads.length} Selected</span>
                    <div className="h-4 w-px bg-white/20"></div>
                    <button className="flex items-center gap-1 text-xs hover:text-primary-300 transition-colors"><span className="material-symbols-outlined text-[16px]">check_circle</span> Mark Converted</button>
                    <button className="flex items-center gap-1 text-xs hover:text-red-300 transition-colors"><span className="material-symbols-outlined text-[16px]">delete</span> Delete</button>
                    <button onClick={() => setSelectedLeads([])} className="ml-2 text-slate-400 hover:text-white"><span className="material-symbols-outlined text-[18px]">close</span></button>
                </div>
            )}
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {filteredLeads.length > 0 ? (
                            filteredLeads.map(lead => (
                                <div
                                    key={lead.id}
                                    className={`p-4 cursor-pointer rounded-xl transition-all border group relative ${selectedLeadId === lead.id ? 'bg-primary/5 border-primary shadow-sm' : 'bg-white dark:bg-[#1A2633] border-transparent hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                                >
                                    <div className="absolute top-4 left-4 z-10" onClick={(e) => e.stopPropagation()}>
                                        <input
                                            ...
                                            className={`size-4 rounded border-slate-300 text-primary focus:ring-primary ${selectedLeads.length > 0 || selectedLeads.includes(lead.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}
                                        />
                                    </div>
                                    <div className="flex justify-between items-start mb-2 pl-6" onClick={() => setSelectedLeadId(lead.id)}>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50 dark:bg-slate-900">
                                {activeTab === 'Overview' ? (
                                    <div className="max-w-5xl mx-auto space-y-6">
                                        ...
                                        {/* 1.1 Assignment (Admin Only) */}
                                        {(currentUser?.userType === 'Admin' || currentUser?.role === 'Manager') && (
                                            <div className="bg-white dark:bg-[#1A2633] p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Assign Lead To</p>
                                                <div className="flex items-center gap-3">
                                                    <div className="relative flex-1">
                                                        <select
                                                            className="w-full appearance-none bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-primary outline-none"
                                                            value={selectedLead.assignedTo || ''}
                                                            onChange={(e) => updateLead(selectedLead.id, { assignedTo: Number(e.target.value) })}
                                                        >
                                                        ...
                                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-500 material-symbols-outlined text-[20px]">expand_more</span>
                                                    </div>
                                                    {selectedLead.assignedTo && (
                                                        <div className="size-11 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 flex items-center justify-center font-black border border-indigo-100 dark:border-indigo-800">
                                                            {staff.find(s => s.id === selectedLead.assignedTo)?.initials}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/10 p-6 rounded-2xl shadow-sm border border-indigo-100 dark:border-indigo-800/50 relative overflow-hidden">
                                            <div className="flex justify-between items-start mb-4 relative z-10">
                                                <h3 className="text-sm font-black text-indigo-900 dark:text-indigo-100 uppercase tracking-wider flex items-center gap-2">
                                                    <Sparkles size={16} className="text-purple-600 dark:text-purple-400" /> AI Lead Intelligence
                                                </h3>
                                                {!selectedLead.aiScore && (
                                                    <button
                                                        onClick={() => handleAnalyzeLead(selectedLead)}
                                                        disabled={analyzing}
                                                        className="bg-purple-600 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-lg hover:bg-purple-700 transition-all flex items-center gap-1 disabled:opacity-50"
                                                    >
                                            ...
                                            {selectedLead.aiScore ? (
                                                <div className="space-y-4 relative z-10">
                                                    <div className="flex items-center gap-4">
                                                        <div className="flex-1 h-3 bg-white dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                                                            <div
                                                                className="h-full bg-gradient-to-r from-teal-400 to-indigo-500 rounded-full transition-all duration-1000"
                                                                style={{ width: `${selectedLead.aiScore}%` }}
                                                            />
                                                        </div>
                                                        <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{selectedLead.aiScore}/100</span>
                                                    </div>
                                                    <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm p-3 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 border border-white/50 dark:border-white/5">
                                                        <p className="leading-relaxed"><span className="font-bold text-indigo-700 dark:text-indigo-300">Strategy:</span> {selectedLead.aiSummary}</p>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="text-center py-2 relative z-10">
                                                    <p className="text-xs text-indigo-400 font-bold uppercase mb-1">Not Analyzed Yet</p>
                                                    <p className="text-sm text-slate-500 max-w-sm mx-auto">Click analyze to get a conversion score and strategic tips based on interactions.</p>
                                                </div>
                                            )}
                                        </div>
