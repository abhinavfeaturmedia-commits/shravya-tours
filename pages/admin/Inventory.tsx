...
// Mock Assets for Inventory Definition
const CAR_ASSETS = [
    { id: 'Sedan', name: 'Premium Sedan', capacity: 5, price: 3500 },
    { id: 'SUV', name: 'Luxury SUV', capacity: 3, price: 5500 },
    { id: 'Van', name: 'Traveler Van', capacity: 2, price: 8000 }
];

const BUS_ASSETS = [
    { id: 'Volvo', name: 'Volvo AC Multi-Axle', capacity: 45, price: 1200 },
    { id: 'Sleeper', name: 'Non-AC Sleeper', capacity: 30, price: 800 }
];

export const Inventory: React.FC = () => {
  const { inventory, updateInventory, packages, bookings } = useData();
  ...
  const getSlot = (day: number): DailySlot => {
      ...
      if (inventoryType === 'Car') {
          asset = CAR_ASSETS.find(c => c.id === selectedAssetId) || CAR_ASSETS[0];
          // Find bookings that match the date, type='Car', and contain the car type in title/details
          matchingBookings = bookings.filter(b => 
              b.type === 'Car' && 
              b.date === dateStr && 
              (b.title.includes(asset.id) || (b.details && b.details.includes(asset.id)))
          ).length;
      } else {
          asset = BUS_ASSETS.find(b => b.id === selectedAssetId) || BUS_ASSETS[0];
          // For buses, we count 'guests' or 'seats' usually, but here we treat 1 booking = 1 seat for simplicity
          matchingBookings = bookings.filter(b => 
              b.type === 'Bus' && 
              b.date === dateStr
          ).reduce((acc, curr) => {
              // Parse guests string "2 Adults" -> 2
              const count = parseInt(curr.guests?.split(' ')[0] || '1');
              return acc + count;
          }, 0);
      }
      ...
      return {
          ...
          price: asset.price,
          isBlocked: false // Logic for blocking cars/buses can be added later
      };
  };
       ...
       <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="px-4 md:px-6 py-4 flex flex-col gap-4 bg-white dark:bg-[#1A2633] border-b border-border-light dark:border-border-dark shadow-sm shrink-0 z-10">
             ...
             <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                   <h3 className="text-xl md:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Inventory & Availability</h3>
                ...
                <div className="flex items-center gap-3">
                   <button onClick={handleExport} className="hidden sm:flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-semibold rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-4 w-full lg:w-auto">
                   ...
                   <div className="relative w-full lg:min-w-[240px]">
                      ...
                      >
                         ...
                         {inventoryType === 'Car' && CAR_ASSETS.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                         {inventoryType === 'Bus' && BUS_ASSETS.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                      </select>
